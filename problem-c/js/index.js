...
console.log(losses.map(extractOpponent));
...
losses.forEach(function(game) {
  const opp = extractOpponent(game);
  let uwScore = 0;
  let oppScore = 0;
  if (game.home === "UW") {
    uwScore = game.home_score;
  ...
  console.log(opp + " at UW, " + oppScore + " to " + uwScore);
});
...
function mostYardsPassing(game1, game2) {
  if (typeof game1.passing_yards === "undefined") {
    return game2;
